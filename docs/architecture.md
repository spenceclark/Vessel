...
| `GET /vessel/api/series` | token series over time for the charts: `metric=tokens_in\|tokens_out\|tokens_total`, `groupBy=none\|tag\|model\|backend`, over the canonical list scope (plain `format`). Oldest-first by id, capped to the newest 5,000 distinct requests with `truncated` + both counts disclosed; tag grouping joins `json_each` so multi-tag requests appear per tag; six-series cap reports `omittedSeries` instead of merging; null-metric rows are excluded by the same predicate so counts reconcile (phase-7) |
...
```jsonc
{
  ...
  "backends": {
    ...
    "typesafe":  { "baseUrl": "https://api.typesafe.ai", "type": "auto", "authEnv": "TYPESAFE_API_KEY" }
  },
...
| TypeSafe | `https://api.typesafe.ai` (443) | System One (`type: auto`) | required | `TYPESAFE_API_KEY` |
