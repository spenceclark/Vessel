...
no endpoint: `AVG(score)`/`COUNT(score)` ride every aggregate row, replay-group win rate is a
second query folded in C# beside the percentiles (a group's members are its `replay_group`
rows plus the original through `replay_of`; ties are wins for everyone at the top), and
...
| `GET /vessel/api/aggregate` | Grouped totals `by=model\|tag\|backend\|format\|patch\|warning` (`by` required — 400 `invalid_request` otherwise; `warning` fans out over the warnings array like `tag` — a request counts once per code): requests/failed, token sums, avg duration/TTFT/tok/s, nearest-rank `p50`/`p95` duration; top 50 groups ranked by tokens then requests, `totalGroups` disclosed |
