...
| `GET /vessel/api/series` | Token series for the context-growth chart: `metric=tokens_in\|tokens_out\|tokens_total`, `groupBy=none\|tag\|model\|backend`, over the canonical list scope (plain `format`, lenient `session`). Newest-first, capped at 5,000 points (`truncated` + both counts), six-series cap with `omittedSeries` counted rather than merged, `estimated` flag when any drawn row is estimated |
